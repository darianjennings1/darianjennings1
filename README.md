<h1> Hi , my name is Darian Jennings </h1>I am an incoming Ph.D. student at The University of Florida.
I'm an engineer who is passionate about elavating people, making open-source more accessible,
and developing efficient & optimal software solutions. <br/> <br/>
Aside from my univeristy studies, I enjoy playing sports such as basketball and soccer,
I love to be outside enjoying nature, and lastly I enjoy coding fun ideas that you can view in my SideProjects repository.
I am currently working on a Senior Design Project for the St. Mary's Unmanned Aerial Systems (UAS) Lab,
for this my team and I are developing a ground contorl station for the lab.<br></br>

Topics I have experience with (to various degrees) include image processing, machine leaarning, web-development, mobile-application development & software testing, and  virtual-reality applciation development & experimenting with human subjects.<br></br>
Some of the programming languages I have used include: C, C++, C#, MATLAB, Python, Java, SQL, Assembly, JavaScript, HTML, & CSS.<br></br>
Software that I have utilized includes but is not limited to: Redis, MySQL, Appium, Android Studio, SolidWorks, Unity, & Toobi.<br></br>

Please connect with me on LinkedIn with the following link:https://www.linkedin.com/in/darianjennings/ <br></br>
<img src="https://cdn-media-1.freecodecamp.org/images/1*HTRqXgr7CVtRBsyTxurQew.jpeg">
<!---
darianjennings1/darianjennings1 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
